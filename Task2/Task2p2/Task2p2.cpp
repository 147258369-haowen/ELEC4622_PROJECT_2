...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        case LAPLACIAM_PYRAMID: {
            ...
            Image_LPF(&input_comps, &temp_comps_inital, &sinc, &imageParam);//lpf
            Image_LPF(&input_comps, &temp_comps, &sinc, &imageParam);//lpf
            Laplacian_difference(&input_comps, &temp_comps_inital, &imageParam);
            Image_copy(&temp_comps_inital, &overlaid, &imageParam);
            Image_DownSample(&temp_comps, &output_comps, &imageParam);//5

            nextinput = output_comps;
           
            for (int i = 0; i < imageParam.D - 1; i++) {
                Image_LPF(&nextinput, &temp_comps, &sinc, &imageParam);//10
                Image_DownSample(&temp_comps, &output_comps, &imageParam);//5
                Image_upsample(&output_comps, &temp_diffback, &imageParam);//10
                Laplacian_difference(&temp_comps, &temp_diffback, &imageParam);//10
